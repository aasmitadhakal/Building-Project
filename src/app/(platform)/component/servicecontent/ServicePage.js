    ...
    <>
      {/* <section className='grid md:grid-cols-3 container mx-auto gap-6 my-12 mb-44  md:px-0 px-4 font-[Montserrat]'>
            {data.map((datas, index) => (
                <>
                <div key={index.id} className='shadow-xl rounded p-4 hover:bg-customblue hover:text-white'>
                    <img  src={axiosInstance.defaults.baseURL + datas.image} alt='img' className='h-[35px] w-[35px] bg-customyellow rounded px-1 py-1'></img>
                    <p className='my-1  font-[600] text-[24px] leading-[32px]' >{datas.short_description}</p>
                    <p className='text-blue font-[400] text-[16px] leading-[25px]' dangerouslySetInnerHTML={{ __html: datas.description }}/>
                </div>
                </>
            ))}
            </section> */}
        ...
        {data.map((datas, index) => (
          <div
            key={index.id}
            className='shadow-xl rounded p-4 hover:bg-customblue hover:text-white'
          >
            <img
              src={axiosInstance.defaults.baseURL + datas.image}
              alt='img'
              className='h-[35px] w-[35px] bg-customyellow rounded px-1 py-1 transition duration-300 ease-in-out'
            />
