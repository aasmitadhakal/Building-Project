...
const Page = () => {
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/pages/${deleteItemId}`);
  ...
  const columns = ["SN", "name", "image","title", "Actions"];
      ...
      <section className="p-5 overflow-x-auto min-w-screen bg-white rounded-md z-10">
        ...
        <div className="max-w-screen-lg w-full">
          <div className="flex justify-between mb-4">
            ...
            <Link href="/dashboard/sitepages/create">
          ...
          {column === "image" ? (
            ...
            item[column.toLowerCase().replace(/\s/g, "_")]
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Link href={`/dashboard/sitepages/${item.id}`}>
